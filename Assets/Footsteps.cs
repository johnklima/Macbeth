...
public class Footsteps : MonoBehaviour
{
	...
	void PlayFootstepSound()
	{
			...
			if (hit.collider.gameObject.layer == LayerMask.NameToLayer("Ground"))//The Viking Village terrain mesh (terrain_near_01) is set to the Ground layer.
			{
				...
				if (materialIndex != -1)
				{
					...
					if (material.name == "mat_terrain_near_01 (Instance)")//This texture name is specific to the terrain mesh in the Viking Village scene.
					{
						...
						m_Forest = maskPixel.a * Mathf.Min(spec2Pixel.a * specMultiplier, 0.9f);//Only the sand texture has water, so we multiply by the mask pixel alpha value.
						m_Grass = (1.0f - maskPixel.a);
						m_Snow = maskPixel.a - m_Forest * 0.1f;//Ducking the sand a little for the water
		...
		if (m_Debug)
			Debug.Log(" Grass: " + m_Grass + " Snow: " + m_Snow + " Forest: " + m_Forest);
		...
		if (m_EventPath != null)
		{
			...
			SetParameter(e, "Forest", m_Forest);
